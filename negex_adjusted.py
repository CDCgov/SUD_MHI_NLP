# -*- coding: utf-8 -*-
"""
Created on Mon Dec  2 12:41:21 2019
A copy-paste of the raw document from here:
https://github.com/chapmanbe/negex/blob/master/negex.python/negex.py    
With some modifications to allow an individual term to trigger negation in more than one position
modifications: Nikki Adams, oxf7@cdc.gov
"""
...
#        print(f"At point 1 sentence is: {self.__sentence}")
        for phrase in self.__phrases:
            ...
            if m:
                self.__sentence = self.__sentence.replace(m.group(0), '[PHRASE]'
                                                          ...
                                                          + '[PHRASE]')
#                print(f"At point 2 sentence is {self.__sentence}")
...
#        POSTNEGATION [POSP].
        ...
        # print(f"Point 3 sentenceTokens {sentenceTokens}")
        if "[PREN]" in sentenceTokens: #if in there once, will be in there twice
            #remove first instance
            sentenceTokens.remove("[PREN]") 
            # print(f"Point 4 sentenceTokens {sentenceTokens}")
            #get position of second instance
            prenIndex = sentenceTokens.index("[PREN]")
            #assume position right before it is trigger to duplicate
            triggerTarget = sentenceTokens[prenIndex-1]
            #strip off tag of target
            target = triggerTarget[6:-6]
            # print(f"This is the target, I think: {target}")
            #duplicate target with PREN
            sentenceTokens[prenIndex]=f"[PREN]{target}[PREN]"
        ...
        for i in range(len(sentenceTokens)):
            # print(f"At sentence token {i} which is {sentenceTokens[i]}")
        ...
        if negP:
            ...
            for i in range(len(sentenceTokens)):
                ...
                if prePossibleFlag == 1 and overlapFlag == 0:
                    ...
                    sentencePortion = sentencePortion + ' ' + sentenceTokens[i]
                #gives TypeError: can only concatenate list (not "str") to list                    
                # sb3 = sb3 + ' ' + sentenceTokens[i]
                # print(f"sb3: {sb3}, sentenceTokens at {i}: {sentenceTokens[i]}")
                sb3.append(' ')
